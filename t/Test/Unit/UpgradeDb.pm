...
sub test_execution_order_subdir_independent {
    my $self = shift;
    ...
    open ARQ, ">/tmp/polvo_test/repository/db/subdir/03.sql";
    print ARQ "update polvo_test_order set x='b' where x='a';\n";
    close ARQ;
    ...
    my ($result) = $self->{DBH}->selectrow_array("select count(*) from polvo_test_order where x = 'e'");
    $self->assert($result == 1, 'sql order was not subdir independent');
